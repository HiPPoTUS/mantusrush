namespace ComputeShaderUtility
{
	...
	public static class ComputeHelper
	{
		...
		public static void CreateRenderTextureB(ref RenderTexture texture, int width, int height, FilterMode filterMode, GraphicsFormat format, Texture2D sourceTexture)
		{
			if (texture == null || !texture.IsCreated() || texture.width != width || texture.height != height || texture.graphicsFormat != format)
			{
				if (texture != null)
			...
			// Если передана sourceTexture, копируем её в RenderTexture
		...
		public static Texture2D LoadBitmap(string filePath)
		{
			byte[] fileData = File.ReadAllBytes(filePath);
			Texture2D texture = new Texture2D(2, 2);
			texture.LoadImage(fileData); // Автоматически определяет размер и формат
			return texture;
		}
